...
from keras.layers import (Conv2D, Input, ZeroPadding2D, Add, Flatten, Dense,
...
def _main(args):
    ...
    for section in cfg_parser.sections():
        ...
        if section.startswith('convolutional'):
            ...
            prev_layer = all_layers[-1]
            prev_layer_shape = K.int_shape(prev_layer)
            weights_shape = (prev_layer_shape[1], output)
            weights_size = np.product(weights_shape)
            print(weights_shape)
            ...
            dense_weights = [dense_weights, dense_bias]
            #print(len(dense_weights))
            count += weights_size 
            count += output
            den_lay = Dense(output, weights=dense_weights, activation=activation)
            all_layers.append(den_lay(prev_layer))   
            prev_layer = all_layers[-1]
